...
- Full Dataiku REST API coverage audit against the official spec (auth, API key permission model, and the exhaustive `dss/api/*` endpoint catalog). Closed the highest-value gaps found:
	...
	- Dashboard insight discovery now captures insight listings and full visual payloads via `list_insights()` / `get_insight()`, so chart-level content is no longer hidden behind dashboard shells
	...
	- Discovery failures for webapps, streaming endpoints, Jupyter notebooks, project libraries, Data Quality status/rules, saved-model versions/details, insights, and project variables each degrade gracefully with a review flag, following the same non-fatal pattern already used for connections/saved models/dashboards
	- Documented, deliberately out-of-scope for now (lower priority, no code changes): Wiki articles and Discussions

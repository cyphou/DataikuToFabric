# Agent Behavior Instructions — Dataiku to Microsoft Fabric Migration
...
- **Pipeline**: Dataiku source → [src/agents, src/analyzers, src/api, src/connectors, src/core, src/drift, src/healers, src/lineage, src/merge, src/models, src/plugins, src/qa, src/reports, src/testing, src/translators, src/__pycache__] → Extraction → [output] → Microsoft Fabric
...
- **Target**: `output/`
- **Tests**: `pytest tests/ --tb=short -q` — 38 test files
